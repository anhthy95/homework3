...
function begin_play(){
	...
	/*5*/
	if(game_started()){
		alert("Already started. Please <strong> Reset Play</strong> to start again.");
		return false;
	}

	if(isEmpty(player1)|| isEmpty(player2)){
		alert("Two player game, both the fields are mandatory."); /*1 Verify if the player names are empty or not */
		return; /*2*/
	}
	else{
		/*3*/
...
*/
function reset_play(){
	/*4 no effect if the game has not begin*/
	if(game_started()){
	/*1*/
	/*Empty text box*/
...
function play() {
	...
	if(game_started()){ /*check if game has started yet*/
		var entry= document.getElementById("move_text_id").value;
		
		
		/*2 */

		if(!isValid(entry)){
			alert("Invalid input! Please enter one of these [A1, A2, A3, B1, B2, B3, C1, C2, C3] again.");
		}
		else{ /*3*/
			var index = table_ids.indexOf(entry);
			if(whose_move()){
				if(board_state[index] == -1){
					...
					alert("INVALID ALERT: This cell is already filled. Choose again");
				}
			}
				...
				if(board_state[index] == -1){
					/*3*/
					board_state[index] = 0;
					document.getElementById(entry).innerHTML = "O";
					document.getElementById("turn_info").innerHTML = "Turn for: <strong> X</strong>";
		...
		if(isWinner()){
			...
			/*7* Reset play after the game is won*/
			reset_play();
		}
	}
	...
	/*6. Validate state after a move*/

	function isWinner(){
	var iswinner = ((board_state[0] == board_state[1]) && (board_state[1] == board_state[2]) && (board_state[2]!=-1) )||
	((board_state[3] == board_state[4]) && (board_state[4] == board_state[5]) && (board_state[5]!=-1) )||
	((board_state[6] == board_state[7]) && (board_state[7] == board_state[8]) && (board_state[8]!=-1) )||
